...
Repeated reads need pacing. On the held USB connection, issuing another current-state request immediately after the startup read timed out after five seconds; the next request then succeeded, producing a deterministic fail/succeed pattern when every CLI call wrote afresh. Waiting six seconds before the next request succeeded. The held daemon therefore retains the last decoded snapshot and spaces current-state requests by at least five seconds. Faster GUI and CLI polls read that snapshot rather than sending duplicate application requests. Forced safety refreshes obey the same interval: on 2026-09-04 an FX write's pre-mutation model check timed out when it sent a current-state request immediately after a sequence of FX reads; waiting out the remainder of the shared interval fixed the path without retrying or weakening the model check. This is a measured Nano rule, not the Quad's push-cache behavior.
...
Gate reduction uses application body `18 0B 20 <varint(percent + 108)> 28 00 1A 00 00 00` for integer percentages from 0 through 100. The raw value crosses the one-byte varint boundary between 19% and 20%; callers must encode the complete varint and reject out-of-range input rather than silently clamp it. Current-state field 53 is a little-endian fixed32 float decoded as `round(value * 255 - 108)`. Field 53 is optional: omission means unknown, not 0%. On 2026-08-22 and 2026-08-26 the connected unit omitted field 53, so the reversible smoke correctly refused before writing. On 2026-09-04 the official editor supplied a readable 49.8% baseline, which the integer protocol model decoded as 50%; the direct USB smoke changed that to 51%, independently read back 51%, restored the encoded integer 50%, and read back 50%. The official MCP-client smoke then passed a same-value Gate write and fresh confirmation through the held daemon. A Tauri-backend test changed the value by one integer point through the same daemon, read it back, restored it and verified restoration in 12.22 seconds. This verifies the integer core USB, daemon/MCP and Tauri-backend paths; the rendered GUI control remains unverified.

Gate/FX bypass is now exposed with the same read-back discipline as amp writes. The device sends an immediate 8-byte acknowledgement with footer `73 00 00 00` after each bypass write, and a fresh state read after the measured six-second settle confirms the new value. The earlier observation (2026-08-18) that a second bypass write on the same HID handle was ignored could not be reproduced on 2026-08-20 across Pre FX 1, Pre FX 2, Post FX 1 and Gate, including a rapid-fire test sending all four toggles in quick succession. The daemon verifies every bypass write with a separately paced current-state request before returning success. One decoder caveat: observed Gate current-state responses omit field 54 in a state thought to be "on," but that relationship is not yet hardware-verified. `NanoSlotState::bypassed` is a tri-state (`Bypassed` / `Engaged` / `Unknown`), not a plain boolean, so the omission decodes to an explicit `Unknown` rather than being indistinguishable from every other role whose current-state message never carries a bypass field at all (Capture). This is a decoder limitation, not a write failure, and omission is not read as proof the Gate is on until NANO-001.14's hardware follow-up passes.

FX parameter refresh uses `08 03 18 <slot> 89 00 00 00`; the response shape is `08 06 22 <byte-length> <little-endian f32 values> 8A 00 00 00`. Array position is the zero-based write index. The response carries neither a requested slot, request ID, parameter name, unit, range nor enum choices. A client must validate the exact footer, finite normalized values and the measured `Device is busy!` confirmation, and serialize refreshes with other Nano operations. Serialization alone cannot distinguish a delayed response from an earlier refresh of another slot: after a refresh times out or returns malformed traffic, close and reopen the HID transport before issuing another request so the stale reply cannot satisfy it. Normalized write values outside 0.0-1.0 or non-finite values must be rejected before device I/O rather than clamped. **Hardware-verified 2026-08-21:** all five editable slots returned non-empty vectors of finite normalized values through the held daemon. A direct reversible test changed one interior Pre FX 1 parameter by `0.01`, independently read back the new value after two seconds, reopened the transport, restored the original and verified restoration. Daemon-backed CLI and Tauri-backend same-value writes also passed fresh read-back. An official MCP client then discovered every Nano tool and passed typed state plus same-value amp, bypass and FX write/read-back checks. Finally, the rendered native Linux GUI loaded a Pre FX 2 vector, changed one slider by `0.001`, showed the device-confirmed result, restored the original through the same control and showed device/draft equality. These operations change heard working state and do not save a preset.

Hosts enrich the raw values as `{ index, name, normalized }` using the typed slot model id and a Rust-owned label profile for each of the 53 known models. The labels and their wire-index ordering are adapted from `rixrix/deskop-nano-cortex` under Apache-2.0; they are not claims about fields observed in the HID reply. Before an FX read, the held daemon refreshes typed current state if its model snapshot is invalid or more than five seconds old, so a known model is never named from stale metadata. Unknown models and values beyond a known profile retain their raw index and value with `name = null`. Callers must always write the explicit `index` and the model id observed when the parameter was loaded; immediately before touching the raw FX parameter path, the daemon waits until another current-state request is permitted, refreshes state, and rejects the write if that model id changed. The GUI also closes an open FX inspector when a refreshed dashboard reports that slot's model changed. Units, display ranges, enum choices and normalized-value conversions remain unmodelled because the available prior-art implementations infer them without per-parameter protocol evidence.
