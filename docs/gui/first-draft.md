...
Gate reduction has its own 0-100% number input and explicit **Apply** action. The Rust boundary rejects larger values and uses the same paced state read-back before reporting success. The integer core USB, held-daemon/MCP and Tauri-backend paths were hardware-verified on 2026-09-04 with independent read-back and restoration; the official editor's 49.8% baseline decoded and restored as the protocol model's integer 50%. This rendered GUI control remains labelled provisional until the exact interaction is exercised against hardware.
