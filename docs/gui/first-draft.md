...
- **Daemon-backed mode**, the real thing: run `s/gui-dev` from anywhere in the repository. The GUI reuses existing compatible product-scoped held sessions or starts missing auto-managed sessions itself, trying Quad and then Nano when no session exists. The header device selector can explicitly choose Quad, Nano, or Auto-detect; if both products are connected, Auto-detect selects Quad, and switching products leaves the other product's held session live.
...
The native package pipeline is configured to build a Linux x86_64 `.deb`, an Apple Silicon `.dmg`, and a Windows x86_64 NSIS `.exe`. Each desktop package embeds the matching `cortex` CLI beside the GUI because that process owns the held USB session; it does not embed `cortex-mcp`. The Linux package installs and activates the two-device udev rule. Published GUI filenames end in `-preview`, release notes repeat their provisional status, and all downloads share one `SHA256SUMS` file that should be verified before installation.

- **Linux x86_64:** `sha256sum --check --ignore-missing SHA256SUMS`, then `sudo apt install ./Cortex_<version>_amd64-preview.deb`. The package reloads udev rules and triggers existing hidraw devices; unplug/replug if the desktop session still has no ACL. This is the only host with real-device evidence.
- **macOS arm64:** set `package=Cortex_<version>_aarch64-preview.dmg`, then run `grep "  $(basename "$package")$" SHA256SUMS | shasum -a 256 -c -`; open the `.dmg` and drag Cortex into Applications. The package is ad-hoc signed, not Developer ID signed or notarized; macOS may require an explicit approval in Privacy & Security. Device and daemon behavior remain provisional.
- **Windows x86_64:** compare `Get-FileHash -Algorithm SHA256` with `SHA256SUMS`, then run the `Cortex_<version>_x64-setup-preview.exe` NSIS installer. It is unsigned and Windows SmartScreen may warn. Installing an upgrade or uninstalling closes only the GUI at that installation path and refuses to continue if any daemon or direct CLI operation still locks the bundled helper; it never stops or kills CLI/session processes implicitly. Native CI tests are configured to cover named-pipe isolation and daemon lifecycle without hardware, but their first remote run remains pending; USB/HID behavior is provisional.
...
Everything below is non-persistent: it changes the working copy the unit is currently playing, exactly as pressing a footswitch or turning a knob on the device does, and saves nothing to storage. **There is no save control in the GUI.** Preset saving remains a CLI operation until its own confirmation UX, restoration semantics and hardware-tested safety contract are implemented here (tracked as GUI-004.1).
...
When the selected daemon owns a Nano Cortex, the GUI renders the fixed eight-role signal chain (Gate, Pre FX 1-2, Capture, IR/Cab, Post FX 1-3) as one ordered row of semantic block buttons, each showing its loaded model name and bypass state. Capture and IR names come from device state; FX model and parameter names are resolved in `cortex-rs` from Nano-specific attributed tables, not guessed from the Quad catalog or implemented again in React. A model id added by newer firmware remains visible as `Unknown model <id>`, and an unknown parameter remains visible as `Param <index>`. Quad and Nano share block-card, selected-state and inspector presentation, but the Nano's roles are never forced into the Quad's grid, scenes or presets. Every role can be selected with pointer or keyboard; editable FX roles expose normalized parameter controls, while the other roles explain that no editable parameters are available yet. Below the inspector, the five hardware-verified raw amp controls (Gain, Level, Bass, Mid, Treble) each get a 0-255 number input and an explicit **Apply** button.
...
Gate reduction has its own 0-100% number input and explicit **Apply** action. The Rust boundary rejects larger values and uses the same paced state read-back before reporting success. This control is labelled provisional: on 2026-08-22 the connected unit omitted Gate-reduction field 53, which means unknown rather than 0%, so the reversible hardware smoke correctly stopped before writing because it could not retain and restore a trustworthy original value. The control and all host boundaries are offline-tested, but the write has not yet been independently hardware-verified.
...
Selecting an FX panel opens its parameter inspector. Editable panels are named button controls and support Enter/Space as well as pointer input; the header device selector is also a named native button rather than a pointer-only badge. Nano bypass controls include the slot and current state in their accessible names, Apply buttons identify the value they submit, and FX sliders identify their semantic name, explicit wire index and normalized range. The inspector first requests the model's normalized parameter values and only enables **Apply** once they are available. Each write carries the model id shown when those parameters were loaded; the daemon refreshes current state and rejects the write before device mutation if the player has since changed that model. Names are attributed host metadata; indices and normalized values are the hardware-verified wire facts, and the GUI does not invent units, ranges, enums or conversions that the reply does not carry. The underlying core and daemon/CLI paths were hardware-verified on 2026-08-21 across reads from all five slots, a reversible write/read-back/restoration cycle and a daemon-backed same-value write. The Tauri backend boundary also passed all five reads and a same-value write/read-back against hardware. The rendered native Linux control then loaded Pre FX 2, changed one slider by `0.001`, displayed the device-confirmed result, restored the original through the same control and displayed matching device/draft values with **Apply** disabled. Amp, bypass and FX parameter edits change heard working state; no Nano operation saves a preset.
