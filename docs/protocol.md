...
Nano writes need operation-specific verification rather than one generic assumption. All five raw amp controls accepted typed writes on one held HID connection and read back exactly after six seconds. An FX bypass write also changed and read back, but a second bypass write on that same connection was ignored despite pacing; closing and reopening the HID handle allowed restoration. Bypass is therefore not safe for a held-session editing surface until that repeat-write behavior is understood.
