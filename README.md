...
- `gui/` - the Tauri 2 + React + Mantine desktop editor, intended for both Cortex devices on Linux, Windows and macOS. It starts or reuses product-scoped held sessions, provides non-persistent Quad editing, and provides a Nano fixed-chain view with raw amp, bypass and FX parameter controls in explicit fixture and daemon-backed modes. The native Nano FX control is hardware-verified. When both products are connected, the selector keeps one live owner per device and switches either direction in under one second after initial startup; automated native DOM/IPC checks and cross-platform packaging remain outstanding.
