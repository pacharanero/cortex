...
That dual-device, cross-platform editor is the destination, not the current support claim. Today the Quad Cortex CLI and MCP paths are hardware-verified on Linux and the GUI supports non-persistent Quad editing. Nano Cortex USB framing, typed current-state decoding, Bluetooth ownership errors, a paced held daemon, state reads, raw amp and bypass editing, and core/daemon/Tauri-backend raw FX parameter editing are hardware-verified. Other Nano operations and untested host integrations remain provisional or unimplemented.
