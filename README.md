...
- `gui/` - the Tauri 2 + React + Mantine desktop editor for both Cortex devices on Linux, Windows and macOS. It starts or reuses product-scoped held sessions, provides non-persistent Quad editing, and provides a Nano fixed-chain view with raw amp, Gate reduction, bypass and FX parameter controls in explicit fixture and daemon-backed modes. The native Nano FX control is hardware-verified on Linux; the rendered Gate control remains provisional. Native `.deb`, `.dmg` and NSIS `.exe` package builds include the sibling `cortex` session helper; Windows and macOS remain tester previews pending hardware verification.
...
Re-plug the device, then `ls -l /dev/hidraw*` should show `crw-rw----+` on the interface-5 node. The installed CLI can hold either product explicitly: Quad uses the full grid/session surface, while Nano currently exposes typed state plus non-persistent amp, Gate reduction, bypass and raw FX parameter operations. Gate reduction is hardware-verified through the core USB, held-daemon/MCP and Tauri-backend paths; its rendered GUI control remains provisional.
