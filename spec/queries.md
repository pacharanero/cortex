...
Absolutely correct - prose is CC, code is AGPL
...
Correct - follow the guitarist-facing Neural docs as this is likely to be recognised by more users
...
Yes use screen number 1 through 4, it is more human. We can include a `--zero-based` option to enable 0..3 indexing, this might be useful when piping commands or for agentic use
...
Clean break, there are no other users
...
I like this: `cortex session start/status/stop` we could retain `cortex connect` as an alias and perhaps have abbreviations eg `cortex c` (do same for all commands, collisions notwithstanding)
...
Yes that would be fine, pick an empty preset, build into the live grid, but stop short of saving.
...
I want to demonstrate the usefulness of a tool like this so the web browsing bit is a big part of it. Having a recipe for a tone that the MCP agent slavishly follows is not as exciting.

3. Is the scratch slot still **2B**, and is that the only slot an agent may write?

An agent must find an Unsaved slot and confirm with the user it si OK to use it, before doing anything
...
I think so, unless another option would be better
...
Tree, but only show populated folders in the default view
...
Love the per-core idea - when reaching DSP limits it's useful to know that there could be strategies to move the work to another core/row/column etc
