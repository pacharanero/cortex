...
| Frame-level decompression is capped at `MAX_DECOMPRESSED_MESSAGE_LEN` (8 MiB) | Implemented, offline-verified | Comfortably above the 150,008-byte largest reassembled body measured on `CorOS` 4.0.1; all concatenated gzip members count toward the limit and a stream that would exceed it returns `Error::Decode` after a bounded read |
...
- [x] A frame-level gzip body that would decompress past `MAX_DECOMPRESSED_MESSAGE_LEN` (8 MiB) is rejected with `Error::Decode` after at most one excess output byte is read; exact-limit input still decodes and concatenated members share the same aggregate limit.
