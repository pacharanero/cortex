...
- **Nano Cortex application coverage is partial.** Low-level transport uses hardware-established PID `0x88E7`, 65-byte reports and shared flag framing. The separate Nano four-byte-footer codec supports current-state reads, hardware-verified non-persistent amp/bypass operations, and provisional FX parameter operations; Quad-envelope requests and sessions still reject Nano before USB I/O so the envelopes cannot be conflated.
