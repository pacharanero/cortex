...
| FR-15 | Reassembly is capped at 1 MiB of reassembled body (`_MAX_MESSAGE_BODY = 1 << 20`). A legitimate message never reaches the cap (the `ModelRepo` reply, ~47 KB gzipped across ~371 reports, is the largest observed). The cap is enforced against actual buffered bytes, both for an in-progress partial and for a just-completed body, not against report count multiplied by an assumed per-report size - a report is never obliged to fill its declared capacity, so a count-based approximation both under- and over-shoots the true byte boundary. A body exceeding the cap, whether mid-reassembly or on the completing LAST/COMPLETE frame, is reset before envelope decoding so the stream resyncs instead of accumulating or decoding an oversized body forever. | Must Have |
...
- [x] Direct fake-link frame-injection coverage proves FIRST-over-partial recovery and exact byte-level 1 MiB cap enforcement, including a LAST frame that only exceeds the cap in its final byte and a body assembled from many short reports well under the cap.
