...
- Direct frame injection proves FIRST-over-partial recovery and exact byte-level 1 MiB cap enforcement: a body at exactly the cap is accepted, a LAST frame that only exceeds the cap in its final byte is rejected before envelope decoding, a body assembled from many short reports well under the cap is not rejected by a report-count approximation, and the loop recovers to deliver a subsequent valid message after either gap.
...
The later `HidLink`/`FakeLink` seam covers the RX loop and writer gate without hardware: reassembled delivery, malformed-report recovery plus cache invalidation, writer priority under a saturated reader, one-second keepalives, and the never-heard liveness guard. The 1 MiB cap and FIRST-over-partial branches were the last narrow unexercised frame-injection cases; direct fake-link tests now cover both.

**Closing that gap found a real bug, not just missing coverage (2026-09-02).** The cap check counted incomplete reports and multiplied by `CHUNK_SIZE` (126) to approximate a byte limit, rather than checking the reassembler's actual buffered byte count. Two consequences: a body built from many reports each carrying far less than 126 bytes could be rejected while still well under the true 1 MiB cap, and - because the check only ran on the *incomplete*-reassembly branch - a LAST frame that pushed a body over the cap in one step was never checked at all and would have reached envelope decoding uncapped. The fix checks `FrameReassembler::buffered_len()` (a new accessor) against the cap for an in-progress partial, and checks the completed body's exact length before `handle_message` decodes it. Both are proven by direct fake-link tests: a body at exactly the cap is accepted, a one-byte-over LAST is rejected before decoding, and a 9,000-report body of 1-byte chunks - which the old report-count check would have rejected past its 8,321st report despite being under 9 KB - is accepted.
