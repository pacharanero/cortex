...
Quad parameter reads attach a Rust-owned `ParameterIdentity` containing the catalog payload generation/revision, live block's model id, catalog parameter's canonical name and positional wire index, and a deterministic kind/range/units/steps descriptor. React returns that identity unchanged with every edit. Daemon protocol 23 carries it as an identified parameter target; `cortex-rs` resolves the active scene first, re-reads the live cell, requires every field to match immediately before sending a grid mutation, and refuses read-back success if the model or catalog identity then differs. The protocol has no atomic compare-and-write operation, so a physical front-panel replacement inside the remaining interval can still make the write unconfirmed; the checks do not claim otherwise. The Tauri backend asks the daemon for the current payload identity on each inspector read and reuses its parsed catalog only while both the lifecycle tokens and exact payload bytes match. This makes catalog replacement, daemon counter reuse, model replacement, parameter reordering and tampered/stale frontend state explicit failures rather than writes through obsolete range metadata.
...
React owns polling, loading/error presentation, selected cell identity, layout and accessibility. It starts the next one-second refresh only after the previous request settles, so the poll loop cannot overlap itself. But the poll is not the only reader: device selection, `switchScene`, scene rename/recolour/bypass, `recall` and the Nano write follow-up each call the dashboard read independently, so their replies can land in a different order than they were issued in. Every one of those reads now takes a `{epoch, seq}` ticket before calling `cortexApi.dashboard()` and applies its reply only if the ticket is still for the current device-selection epoch and newer than the last one accepted for that epoch (GUI-001.9) - so an earlier-issued read settling late can overwrite neither the snapshot, the generation, the selection nor the error state. Selection is stored as `{row,column}` and resolved against the newest snapshot; stale block objects are never retained. Device choices carry their own epoch, bumped by `switchDevice`, so a slower earlier request cannot overwrite the final selection; their dashboard reads also use the shared ticket gate so same-epoch actions started while a switch is pending cannot restore the previous dashboard afterward. Reconnecting state shows the daemon's attempt count and last error plus a **Reconnect now** action; it does not invent a determinate progress bar because the daemon uses bounded exponential backoff rather than a fixed-duration operation.

`SceneSelector` (GUI-006.3) takes the same generation/revision pair as explicit props and uses them, alongside the reported `active_scene`, to tell a genuinely newer device-reported scene transition apart from an unrelated poll, a metadata-only revision bump, a stale reply, and a reconnect's replacement generation - the same distinctions DES-SNAPSHOT draws for the whole dashboard, applied here to one polite-region announcement rather than to the read gate. A switch this control itself requested still announces exactly once despite its own read-back landing as a later, scene-changing report: a ref set before the request is awaited is consumed by the next matching report instead of firing a second, possibly conflicting, announcement - and a device-reported transition never moves focus, only the user's own actions do.

The daemon-backed GUI shows health, cache generation/revision, the Quad live grid, active scene, CPU total and per-core columns, populated Quad setlist slots, and the Nano fixed signal chain. Quad and Nano retain separate domain models but share semantic editor-canvas block cards, operational-state text, selection treatment and inspector framing. Nano FX display names arrive in the typed snapshot after Rust resolves the Nano-specific model id; the webview only chooses the device-supplied Capture/IR name, resolved FX name, or explicit unknown-id fallback. Product-specific navigation is rendered only where its typed data and operations exist: Nano does not reserve the Quad preset sidebar. At the supported 800x600 minimum, all eight Nano roles fit in one row; below a 768px effective viewport, including high zoom, the chain scrolls rather than reflowing into false routes. It exposes typed non-persistent recall and working-state edits for the implemented Quad and Nano operations. Persistent save/delete actions remain outside this milestone until the GUI implements the shared safety contract and confirmation UX.
...
Debug builds also register the `tauri-plugin-mcp-bridge` automation surface behind `#[cfg(debug_assertions)]`, bound to `127.0.0.1:9223` rather than the plugin's default `0.0.0.0`, with `withGlobalTauri` and an `mcp-bridge:default` capability for the main window. This is the driver the Tauri MCP server (hypothesi/mcp-server-tauri) connects to for automated DOM/IPC checks - the GUI-001.6 outstanding item - and it let the GUI-003.10 rendered-control hardware smoke drive the running Linux webview without stealing focus. Release builds contain none of it.

## [DES-CAPABILITY] Internal Capability Evidence Defaults To Unverified

`gui/src-tauri/src/capability.rs` carries the honest verified-vs-provisional evidence AGENTS.md calls for, following the `deskop-nano-cortex` precedent recorded in [prior-art.md](../prior-art.md#the-idea-most-worth-stealing). `CapabilityStatus` is `confirmed-readable`, `confirmed-writable`, `inferred`, `unsupported`, or `unverified`, and `unverified` is the `Default` - both for the enum and for `CapabilityMatrix::status` on any operation key the matrix does not contain. The matrix is host-aware: only Linux has real-device GUI/Tauri evidence, so the current Windows and macOS builds receive no confirmed entries. The type keeps unknown engineering evidence unverified rather than confirmed by omission; that maturity metadata is not repeated beside ordinary player controls.

`default_matrix()` is seeded only from operations whose current Tauri command contract has a recorded hardware pass on that host elsewhere in this repository (`spec/roadmap.md`), never from "it is implemented" or "it passed offline/fixture verification". This criterion consistently confirms both Nano amp and Gate reduction on Linux because each current Tauri boundary passed an independent write/read-back/restoration cycle; the rendered Gate interaction remains a separate unexercised presentation path. `set_bypass`, `set_scene_label`, `set_scene_color`, `copy_scene`, and `set_nano_bypass` stay unverified because their broader core/daemon hardware evidence does not itemise the GUI/Tauri command path. `read_nano_fx_params` and `set_nano_fx_param` also stay unverified: their 2026-08-21 Tauri/rendered smoke predates NANO-001.9's descriptor response and mandatory expected-model safety field, and offline tests of that changed contract cannot inherit the old status. Promoting an operation requires editing this seed alongside the roadmap entry that supplies current command-contract and host evidence.

`pub const OPERATIONS` names every device-operation surface this matrix records - the Tauri command name doubling as the lookup key - and `labels()` maps it against `default_matrix()` to produce the complete `Vec<CapabilityLabel>` available from the typed diagnostic boundary; an operation named in `OPERATIONS` but absent from the seed still appears as `Unverified`. Host lifecycle controls (`dashboard`, `reconnect_now`, and `set_device`) are deliberately outside this read/write device matrix rather than inaccurately described as non-interactive operations.

The `capability_matrix` Tauri command returns `labels()` directly - pure computation with no device I/O, so it needs no `spawn_blocking` - and remains available to diagnostics and future support tooling. The ordinary editor does not fetch or thread this list and has no `CapabilityBadge`: development maturity belongs in the roadmap, diagnostics and release notes rather than repeated player-facing chrome. User-visible refusals and unavailable actions still come from the typed operation boundary, so removing maturity badges does not make an unsupported operation appear usable.
