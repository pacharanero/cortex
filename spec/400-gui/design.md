...
`pub const OPERATIONS` names every device-operation surface this matrix labels - the Tauri command name doubling as the lookup key - and `labels()` maps it against `default_matrix()` to produce the complete `Vec<CapabilityLabel>` the production frontend consumes; an operation named in `OPERATIONS` but absent from the seed still appears, labelled `Unverified`, so the frontend never has to guess which device operations exist. Host lifecycle controls (`dashboard`, `reconnect_now`, and `set_device`) are deliberately outside this read/write device matrix rather than inaccurately described as non-interactive operations.

The `capability_matrix` Tauri command returns `labels()` directly - pure computation with no device I/O, so it needs no `spawn_blocking` - and every rendered control that corresponds to an `OPERATIONS` entry displays a `CapabilityBadge` (`gui/src/shared/CapabilityBadge.tsx`) fed from one `cortexApi.capabilities()` fetch, cached in `App.tsx` state and threaded down as a plain prop rather than re-fetched per panel. The frontend holds no independent copy of which operations are confirmed: fixture mode returns no labels rather than hand-maintaining a second policy, and a component that receives no `capabilities` prop (every consuming component defaults it to `[]`) renders every operation as unverified, the same safe default the Rust side enforces. Each badge visibly names its operation before the textual status (`"Parameter read: Hardware-verified (read)"`, `"FX parameter write: Not yet hardware-verified"`, ...) so adjacent labels remain distinguishable without colour or visual proximity, per GUI-006.1. Auditing the seed against `spec/roadmap.md` for GUI-004.2 also found `docs/gui/first-draft.md` claiming Quad bypass was hardware-verified alongside parameter edits; that document has been corrected to match the matrix.
