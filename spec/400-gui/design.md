...
React owns polling, loading/error presentation, selected cell identity, layout and accessibility. It starts the next one-second refresh only after the previous request settles, so the poll loop cannot overlap itself. But the poll is not the only reader: `switchScene`, scene rename/recolour/bypass, `recall` and the Nano write follow-up each call the dashboard read independently, so their replies can land in a different order than they were issued in. Every one of those reads now takes a `{epoch, seq}` ticket before calling `cortexApi.dashboard()` and applies its reply only if the ticket is still for the current device-selection epoch and newer than the last one accepted for that epoch (GUI-001.9) - so an earlier-issued read settling late can overwrite neither the snapshot, the generation, the selection nor the error state, whether it succeeds or fails. Selection is stored as `{row,column}` and resolved against the newest snapshot; stale block objects are never retained. Device choices carry their own epoch, bumped by `switchDevice`, so a slower earlier request cannot overwrite the final selection; the read-ticket gate resets per epoch and does not change that mechanism. Reconnecting state shows the daemon's attempt count and last error plus a **Reconnect now** action; it does not invent a determinate progress bar because the daemon uses bounded exponential backoff rather than a fixed-duration operation.
