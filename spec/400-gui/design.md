...
`default_matrix()` is seeded only from operations with a recorded hardware pass elsewhere in this repository (`spec/roadmap.md`), never from "it is implemented" or "it passed offline/fixture verification". `set_bypass`, `set_scene_label`, `set_scene_color`, `set_nano_gate_reduction` and `set_nano_bypass` are the worked example: all five exist and are exercised by Rust, fixture or component tests - and all five stay `unverified` here. The first three are offline-verified only (GUI-003.3, GUI-003.4); the Nano pair each have *some* hardware evidence in `spec/roadmap.md` (NANO-001.5/.6), but neither citation is the exact rendered-GUI-control pass this matrix requires - the Gate-reduction entry explicitly records "the rendered Gate control remains provisional", and the bypass entry never itemises a Tauri/rendered pass the way the amp and FX operations do. Promoting an operation requires editing this seed alongside the roadmap entry that supplies the unambiguous hardware evidence, not on the strength of the code appearing to work or of a related sibling operation being confirmed.

`pub const OPERATIONS` names every operation surface this matrix labels - the Tauri command name doubling as the lookup key - and `labels()` maps it against `default_matrix()` to produce the complete `Vec<CapabilityLabel>` the frontend consumes; an operation named in `OPERATIONS` but absent from the seed still appears, labelled `Unverified`, so the frontend never has to guess which operations exist. `dashboard` and `reconnect_now` are deliberately excluded from `OPERATIONS`: they are the always-on read/health path rather than a discrete user-triggered operation with its own evidence claim.

The `capability_matrix` Tauri command returns `labels()` directly - pure computation with no device I/O, so it needs no `spawn_blocking` - and every rendered control that corresponds to an `OPERATIONS` entry displays a `CapabilityBadge` (`gui/src/shared/CapabilityBadge.tsx`) fed from one `cortexApi.capabilities()` fetch, cached in `App.tsx` state and threaded down as a plain prop rather than re-fetched per panel. The frontend holds no independent copy of which operations are confirmed: a component that receives no `capabilities` prop (every consuming component defaults it to `[]`) renders every operation as unverified, the same safe default the Rust side enforces. Text conveys the status (`"Hardware-verified"`, `"Hardware-verified (read)"`, `"Not yet hardware-verified"`, ...) rather than colour alone, per GUI-006.1. Auditing the seed against `spec/roadmap.md` for GUI-004.2 also found `docs/gui/first-draft.md` claiming Quad bypass was hardware-verified alongside parameter edits; that document has been corrected to match the matrix.
