...
`cortex session start` owns one subscribed connection for the selected product and serves typed line-delimited requests over product-scoped local IPC. Quad retains the legacy `cortex.sock`; Nano uses `cortex-nano.sock`, with derived lock and log paths. `session status` and `session stop` default to Quad for compatibility and accept `--device nano` for the Nano endpoint. Ordinary Quad commands use the Quad daemon when available and otherwise open one bounded direct session; Nano commands route only to the Nano daemon. Each endpoint is claimed before its handshake so a second current-version process cannot race startup and wedge that physical device. A Nano start also probes the legacy Quad endpoint and refuses to open HID when an older Nano daemon already owns it. Simultaneously launching old and new binaries is unsupported because the older process cannot participate in the product-scoped ownership contract; stop all pre-upgrade daemons before starting the new version. Distinct Quad and Nano daemons may coexist because they own distinct USB interfaces; the invariant is one owner per physical device, not one owner for the whole host. `cortex-host` owns the platform seam and concurrent accept/request lifecycle: owner-only Unix domain sockets and local-only Windows named pipes with an owner-only DACL. Unix uses `XDG_RUNTIME_DIR` when present and otherwise derives the shared temporary path from the effective OS UID, never mutable `UID` or `HOME` environment values. Windows queries the current process token directly and hashes its account SID into stable per-user communication and ownership-pipe names; holding the one-instance ownership listener gives daemon and direct access the same process-lifetime exclusion without depending on a mutable temporary-directory path. Clients open pipes with anonymous security quality-of-service and accept a server only when its process token carries the same account SID, preventing a differently privileged local account from impersonating either endpoint. A missing Windows pipe returns immediately unless its ownership claim proves that startup is in progress. Cached values are served only while their generation is usable; missing values fall back to explicit reads.

Nano current-state requests are rate-limited independently of FX parameter reads. Polling serves a snapshot for five seconds, and a forced safety refresh waits out any remainder of that interval while holding the exclusive operation gate. This matters before an FX write: model identity must be refreshed immediately before mutation, but sending that state request too soon after the previous one can produce a silent timeout. Waiting preserves both the measured device pacing and the model-change refusal; retrying would risk consuming an uncorrelated delayed response.
...
- **Windows remains hardware-provisional.** The daemon/client expose no platform socket types outside `cortex-host::ipc`; owner-only named pipes, process-lifetime claims, timeout behavior, detached daemon startup and native process tests are implemented. The native Windows GUI-preview job must pass and the CLI-through-device USB/HID smoke must run on real hardware before Windows is supported.
- **Windows pipe names remain deterministic.** A different local account can pre-create the global communication or ownership-pipe name and deny startup. Explicit current-account DACLs, anonymous client SQOS and server-token verification prevent that process from reading commands, impersonating the client or supplying accepted responses, but eliminating this availability-only pre-claim requires a private namespace or owner-only rendezvous secret (CLI-004.12).
