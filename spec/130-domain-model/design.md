...
Hardware confirms Nano VID:PID `152A:88E7`, 65-byte HID reports, shared flag framing, and its separate four-byte-footer codec. Typed state, amp, bypass and raw FX parameter operations are hardware-verified; wider operations are capability-gated rather than making the device kind itself provisional. `DeviceKind` carries the PID and geometry, while the Quad message request and session paths reject Nano before USB I/O so its four-byte footer can never reach the Quad eight-byte parser.
