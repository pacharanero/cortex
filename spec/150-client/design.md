...
The public client sends no Neural Capture dialog response. A 2026-08-12 CorOS 4.0.1 probe sent the exact false-only `NeuralCapture{UPDATE, show_dialog: false}` after a decoded `try_to_show_dialog: true`; the unit froze, stopped responding on HID, and rebooted. Causation is not established, but the response path and its tests were removed rather than treating false as graceful. Capture creation, dialog handling, transfer, backup, and cloud workflows are native-device operations; selecting an existing capture remains supported. Positive acceptance remains unavailable to CLI, MCP and GUI.

Offline fake-link tests pin exact shapes, order, validation-before-I/O, existing-block reuse, placement-refusal short circuit, no unsolicited capture-dialog response, and unrelated-message rejection. Hardware tests selected one existing library capture and one existing user IR without publishing identities, fresh-read the exact strings, verified a post-capture VOLUME write, never saved, and unconditionally recalled before disconnect. A timed on-unit inspection confirmed no warning for the selected user IR. This completes PROT-006.11. Export/import remains separate under PROT-007; host-owned Capture lifecycle is explicitly descoped under FUTURE-008.
...
`fetch_model_repo()` returns the payload captured by the paced handshake, avoiding a second 46 KB device transfer; without a current captured copy it performs `ModelRepo` READ + `await_broadcast`. `Catalog::parse` turns integer model ids into names, categories, and parameter lists in wire-index order. It covers installed block types, including purchased plugins. Neural Capture models are capture block types; individual capture inventory comes from separate `File` listings. The persistent daemon parses its captured copy once per exact generation/revision and evicts the parsed form when the reducer invalidates the payload. Its `Catalog` response carries that payload generation/revision, allowing a host to reuse a parsed local copy only while the daemon still names the same source payload.

Name and identified resolution belong to `QuadCortex::set_parameter`, not a host surface. It resolves the active scene before the pre-write read, reads the live cell to discover the model, resolves the parameter through the catalog, and converts a real-unit value through that parameter's declared range. An identified target also carries the catalog payload generation/revision, model id, positional index, canonical name and a deterministic kind/range/units/steps descriptor observed by the host; all must still match immediately before mutation, and read-back refuses to report success if either the model or catalog identity then differs. The descriptor closes the daemon-restart case where lifecycle counters can repeat. This is the strongest available non-atomic check: the protocol has no compare-and-write guard, so a physical front-panel change inside the remaining validation/write/read-back interval produces an unconfirmed result rather than an atomic guarantee. The CLI, daemon, MCP server and GUI therefore share one implementation. APIs whose existing signatures cannot obtain a fresh live cell and catalog reject identified targets rather than claim stale-state protection from caller-supplied data.

List option resolution belongs to `QuadCortex::set_param_option`. Its caller supplies the current preset because dynamic option names can enumerate blocks and therefore cannot come from static catalog metadata. The target may be an index or catalog name; the client obtains the source block's model when needed, then delegates dynamic-list lookup and `index / (count - 1)` normalization to zone 130 helpers before issuing the ordinary parameter write. An identified target is refused before I/O because this API cannot prove the supplied preset is still live.
