...
The pure `build_decline_capture_dialog` constructor has no Boolean argument and can encode only `NeuralCapture{UPDATE, show_dialog: false}`. `decline_capture_dialog` uses the session's register-before-trigger waiter to accept only a decoded `try_to_show_dialog: true` after the operator prompt becomes active, then installs a collector before sending false and returns the observation window's NeuralCapture traffic. It neither sends nor interprets `show_dialog_fail_reason`. Positive acceptance is absent from the public client and remains unavailable to CLI, MCP and GUI until a complete v1/v2 host capture UI exists.
