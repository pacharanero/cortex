...
| FR-34 | `set_param(row, column, param_index, value)` is the low-level wire-index write. `set_parameter(row, column, target, input, scene, promote, timeout)` is the shared host-facing API: it validates rows/columns/scenes and normalised values, resolves the target scene before reading the cell's model, resolves named and exact prior-read identities through the device catalog, refuses meters, converts real units, performs the 3-message scene sequence when requested, and confirms the target scene/value through a complete live-grid read. An identified target binds the catalog payload generation/revision plus model id, positional index, canonical name and deterministic kind/range/units/steps descriptor, so changed conversion metadata fails before mutation even if daemon lifecycle counters repeat; read-back also refuses success if the model or catalog identity changed during the write. These checks are deliberately non-atomic because the wire protocol exposes no compare-and-write operation, so a physical front-panel change in the remaining interval can still make the outcome unconfirmed. `set_param_option` resolves an index or catalog name against dynamic options in a supplied current preset and centrally normalizes the named option; it rejects identified targets because caller-supplied preset state cannot establish live identity. | Must Have |
...
| FR-50 | `set_expression(row, column, param, pedal, minimum, maximum, model)` assigns an expression pedal to an index or catalog-named parameter. It rejects identified targets because the dispatch-only API has no live-cell/catalog read. | Should Have |
