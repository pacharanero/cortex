...
Raised by a review of the state-cache/prepared-save/daemon-routing change (`a603bce`). The tree was green - fmt, clippy, `reuse`, and 149 tests all passed - so every item here is a behavioural gap that the existing tests do not reach. **009.1 and 009.2 have been independently confirmed by reading the code; the rest are the reviewer's claims and need checking before they are acted on or dismissed.**

Ordered by what would hurt a user most.

- [ ] **PROT-009.2**: **The shipping save path bypasses the prepared-save API entirely.** CONFIRMED 2026-08-05 by call-site search. `save_current_preset` has exactly two production callers - `crates/cortex-cli/src/main.rs:1267` (direct CLI) and `crates/cortex-cli/src/connect.rs:427` (daemon handler) - and both call it raw, with no preparation, scratch policy, backup or confirmation. `prepare_save_before_editing` and `save_prepared` have **no callers outside `safety.rs`**: every use is in its own test block. The safety module is 792 lines wired to nothing. So the safety layer this change added does not protect the surfaces a user actually touches, and a non-`/opt/neuraldsp/` slot can still be overwritten with no backup. **Fix before anything describes prepared saves as a guarantee.** Referred to elsewhere as PROT-006.10.1
...
- [ ] **PROT-009.4**: **An eventually consistent listing is treated as proof a slot is empty.** Preparation skips the backup when one listing says the target is empty, and the commit compares one more listing - but `docs/protocol.md` records that listings can stay stale for over five seconds after a mutation. A recently filled slot can read empty twice, get no backup, and be overwritten. Poll for the expected state instead of trusting a single read
...
- [ ] **PROT-009.8**: **A malformed envelope is skipped without breaking cache continuity.** A failed `Message::decode` is traced but does not call `stream_gap`, unlike a malformed HID frame. The message type is unrecoverable, so the lost envelope may have carried state while the cache stays `Live`
...
- [ ] **PROT-009.10**: **The daemon socket protocol changed without a version gate.** Several request shapes changed and new ones were added, but `daemon_version` is never checked by the client and both old and new binaries report `0.0.1`. Upgrading the CLI while a detached daemon survives gives parse errors with no fallback and no guidance to restart
- [ ] **PROT-009.11**: **`SavePolicy::is_user_setlist` accepts dot path components.** It only requires a non-empty remainder containing no slash, so `.` and `..` pass; `..` would escape the configured USER root if the device normalises paths. Violates the function's own direct-child contract either way
- [ ] **PROT-009.12**: **Named parameter resolution does not enforce the catalog's value type.** After resolving a name it refuses meters but then accepts `Text` or `Normalised` regardless of `ParameterKind`, so text can be sent to a numeric parameter and a float to a string one. The device stores whichever variant arrives; reject before the wire
- [ ] **PROT-009.13**: **An empty prepared target can be saved with no name.** `name: None` means "save in place under the existing name", which an empty slot does not have. `save_prepared` does not check `target_was_empty` before accepting it; the CLI documents the requirement without enforcing it
...
**The prepared-save API exists but nothing enforces it yet.** The CLI and daemon still call `save_current_preset` directly, so a GUI that wants target backup, scratch policy and revalidation must call `safety.rs` itself rather than assume the layer beneath provides them. Wiring the shipping surfaces onto it is tracked as PROT-006.10.1.
