...
- [x] **CLI-004.13**: Added `cliff.toml`: git-cliff generates `CHANGELOG.md`; cargo-dist does not generate changelogs itself, it only optionally consumes an existing file (confirmed against cargo-dist's own `package.changelog` reference), so git-cliff is the one source. `s/version++` already called `git cliff --tag ... --output CHANGELOG.md` whenever `cliff.toml` existed but never checked it was installed; it now pins and verifies `git-cliff 2.13.1` first, matching how `s/markdownlint` pins its tool. `.github/workflows/release.yml`'s `host` job now checks out `CHANGELOG.md` at the release tag and passes the exact `## [x.y.z]` section to `gh release create --notes-file`, falling back to `--generate-notes` only when that section does not exist yet (true until the next real version bump). **Residual:** the config was adapted from git-cliff's own published default (`github.com/orhun/git-cliff/blob/v2.13.1/config/cliff.toml`) rather than the `~/code/discourse/dsc` living example named in the original annotation, because that path does not exist in this sandbox; grouping was then verified by generating a real (uncommitted) preview against this repo's actual history with `git cliff --tag v0.1.0`, which correctly grouped `feat`/`fix`/`docs`/`nightly`/`chore`/`ci`, skipped `chore(deps...)` noise and the one non-conventional merge commit, and extracted the scope from `nightly(ID): ...` subjects. No CHANGELOG.md is committed yet since no version has shipped; it is generated at the next `s/version++` run
