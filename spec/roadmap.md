...
Raised by a review of the state-cache/prepared-save/daemon-routing change (`a603bce`). The tree was green - fmt, clippy, `reuse`, and 149 tests all passed - so every item here is a behavioural gap that the existing tests do not reach. **Only 009.1 has been independently confirmed by reading the code; the rest are the reviewer's claims and need checking before they are acted on or dismissed.**

Ordered by what would hurt a user most.

- [ ] **PROT-009.2**: **The shipping save path bypasses the prepared-save API entirely.** The daemon exposes a raw `SavePreset` request with no preparation, scratch policy, backup or confirmation, and its handler calls `save_current_preset` directly; the CLI does the same. `prepare_save_before_editing` and `save_prepared` are called only from tests. So the safety layer this change added does not protect the surfaces a user actually touches, and a non-`/opt/neuraldsp/` slot can still be overwritten with no backup. **Fix before anything describes prepared saves as a guarantee.** Referred to elsewhere as PROT-006.10.1
