...
- [~] **ENG-007.1**: Correct Nano daemon request contracts and state bookkeeping. `NanoReadFxParams` must never return a cached `NanoCurrentState` where its callers decode `Vec<f32>`; Nano FX reads and writes must update health and the cached fresh-state metadata consistently; and all Nano writes must preserve typed `Busy`, device and outcome-unconfirmed failures. Add daemon-protocol JSON and request-handler regression coverage
- [ ] **ENG-007.2**: Consolidate the duplicated MCP and Tauri daemon supervision logic behind one `cortex-host` owner-aware helper, preserving compatible-session reuse, stale endpoint cleanup, detached lifecycle and actionable version-skew errors
- [ ] **ENG-007.3**: Remove duplicate Nano read/reassembly loops by giving the leaf transport one tested message-read primitive. Reuse `Message::decode` where its Quad envelope semantics apply; retain Nano's distinct four-byte footer codec rather than pretending the two protocols are identical
- [ ] **ENG-007.4**: Replace Nano's concrete `&Transport` dependency with the existing `HidLink` seam, then add deterministic request/reply, timeout and unrelated-message tests without a physical device
- [ ] **ENG-007.5**: Add daemon-protocol JSON shape coverage for every `Request` variant, and Nano dry-run/fixture coverage to CI. The fixture must exercise the production request contract rather than duplicating a looser approximation
...
- [~] **NANO-001.7**: Audit correctness follow-up for the Nano FX and bypass surface. The FX parameter refresh must validate its measured `0x8a 00 00 00` footer and associate a reply with the requested slot before accepting values; a queued response from another slot must not silently verify the wrong write. MCP must require the `bypassed` argument instead of treating omission as `false`, and device-agnostic tools must route to Nano sessions by capability rather than name-prefix convention. The GUI must route Nano writes through its write-in-progress guard and suppress polling while an amp draft is dirty, so a one-second refresh cannot replace a user's unsubmitted value. Add regression coverage before the next hardware pass
