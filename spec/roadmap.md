...
- [ ] **PROT-008.6**: `cortex connect` - a persistent, subscribed session. **In progress: the session holds and serves, the cache and health work do not exist yet.** Cortex Control is fast because it opens ONE session and keeps it; we pay a handshake per command. It is also the right shape for the MCP server, which must hold a single connection anyway, and for the GUI. Measured against a held session: `scene` 0.07 s, `grid` 0.14 s, `--status` 0.005 s, against a 3-5 s direct baseline.
  ...
  - [ ] **008.6.4**: **Health reporting.** Detect a dropped or unresponsive device and say so, rather than hanging. Reconnect with backoff, reporting each attempt. `Session::seconds_since_last_message()` now supplies the signal (the ~0.8 s `GlobalTempo` heartbeat makes silence conclusive), but nothing yet acts on it: a request against a silent device still burns the full 30 s timeout, and `status` reports the raw number without a verdict
  ...
  - [ ] **008.6.7**: Commands address the daemon when it is running. `scene` and `grid` do; the rest still open the device for themselves. **Correction, hardware-verified:** the original plan had `version` keep addressing the device directly, on the reasoning that it needs no handshake. It cannot. A direct open alongside a held session wedges the device (see 008.6.9), and `version` was the command that proved it. It must route through the daemon when one is running - `Request::Version` already exists - and go direct only when none is
  - [x] **008.6.8**: Fall back to a direct `Minimal` session when no daemon is running, so single commands still work standalone
  - [x] **008.6.9**: **Refuse to open the device while a daemon holds it.** Hardware-verified the hard way: a one-shot command that opened the device alongside the held session left the device silent - no reply, and no `GlobalTempo` heartbeat either - and every later read on the held session timed out. Nothing errored at the point of the collision. `ensure_device_free()` is the CLI-side expression of the exclusive-access invariant. The socket is bound *before* the handshake, because the handshake window (33 s on an unsettled device) is when the daemon owns the interface but a late `bind()` would leave `is_running()` answering false
  - [ ] **008.6.10**: **Establish what handshake time actually depends on.** Measured 7.4 s, then 32.9 s, then 52.1 s on successive daemon starts. The leading hypothesis is **DSP load: the unit was being played during these runs.** The device prioritises audio, and the subscribed handshake's ~600 KB state dump is exactly the sort of bulk transfer that would be starved by it. An earlier reading of the same numbers blamed residue from the collisions in 008.6.9; that was under-determined - the collisions were merely what was being looked at - and `pyquadcortex` had already ruled out plain reconnection as a cause (12 abandoned sessions, no degradation). Do not treat either explanation as established.
    - The test: power-cycle for a baseline, then measure repeated clean start/stop cycles with the unit **idle**, then repeat while **playing**, then repeat while playing a **deliberately heavy preset**. No collisions in any arm.
    - Why it matters beyond curiosity: every timeout in the crate (10 s, 15 s, 30 s) was chosen against an idle unit. The 52.1 s run still succeeded because those budgets are per-step rather than total, but that is luck, not design. If handshake cost tracks DSP load then the budgets need to be justified against a loaded unit, and a player is exactly the person who will be running one
