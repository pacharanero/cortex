...
- [~] **ENG-004.1**: Add `@see` traceability headers to all owned source files linking to zone specs. **Night: slice.** `crates/cortex-rs/src/`, `crates/cortex-cli/src/`, `crates/cortex-host/src/`, `crates/cortex-mcp/src/`, and `gui/src-tauri/src/` now have top-level headers. The stale-link sub-item is done: `s/check-traceability` (ENG-004.2) surfaced seven headers pointing at IDs that had moved or been split into subsection IDs during a design-doc restructure. `client.rs` and `session.rs` now use their closest living design anchors; the other five now point to their governing zone specs/designs rather than progress ledgers. Three pre-existing progress-only headers found during review also gained living targets. The eight owned Rust build/test files that `rg --files-without-match '@see' --glob '*.rs'` used to report - `crates/cortex-rs/{build.rs,tests/hardware-reads.rs}`, `crates/cortex-cli/tests/{lifecycle-hardware.rs,file-ops-process.rs}`, `crates/cortex-host/tests/{daemon-lifecycle-process.rs,daemon-protocol-skew-process.rs}`, `crates/cortex-mcp/tests/server-process.rs` and `gui/src-tauri/build.rs` - now carry headers citing the FR/DES ids their coverage matches most precisely, verified against `s/check-traceability`. `rg --files-without-match '@see' --glob '*.rs'` now reports nothing. **Remaining:** explicitly settle whether FR-1's generic "source files" includes the owned TypeScript/frontend, scripts and configuration surfaces before declaring complete. The acceptance criterion currently requires every owned `.rs` file, including build scripts and tests, which is now satisfied; do not narrow that contract implicitly when extending it to non-Rust surfaces
