...
The typed read/edit, live-cache, health, reconnect, and shared prepared-save foundations are in place. The interactive Tauri GUI has explicit fixture and daemon-backed modes with generation-checked Quad and Nano state plus bounded non-persistent working-state operations. Its production Rust boundary and physical unplug/reconnect behavior have passed against a real held session in the native Linux window; automated Tauri DOM/IPC checks and dual-device rendered inspection remain outstanding. Save controls remain absent until the GUI implements exact-target preparation and confirmation UX, restoration semantics, typed failures and its own persistent-write hardware smoke. See [400-gui/spec.md](400-gui/spec.md).
