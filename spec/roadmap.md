...
Saving remains outside this milestone. GUI save needs exact-target preparation and confirmation UX, restoration semantics, typed failures and its own hardware smoke. The Linux x86_64 release path is complete; Windows/macOS host expansion remains under CLI-004.12 and CLI-004.14.
...
- [~] **CLI-004.12**: Keep the Windows door open. Done: `cortex-host` exposes transport-neutral `LocalEndpoint`/`LocalListener`/`LocalConnection`; the Windows adapter uses local-only named pipes with an explicit current-account ACL, token-derived endpoint names, non-impersonable client handles, same-account server authentication and process-lifetime ownership claims; detached daemon startup uses Windows process flags; daemon process tests passed natively in release-preview run `33317851191`. Outstanding: prevent a different local account from denying startup by pre-creating the deterministic global pipe name, then hardware-test the HID/session path on Windows
...
- [~] **CLI-004.14**: Native Tauri jobs build a Linux x86_64 `.deb`, Apple Silicon `.dmg` and Windows x86_64 NSIS `.exe`, each with its target-qualified `cortex` helper staged as a sibling binary. Windows uses current-user named-pipe ACLs and detached daemon startup; native host/CLI tests run before packaging. macOS is ad-hoc signed and Windows is unsigned, and both artifacts are labelled provisional. Release-preview run `33317851191` passed all three native package jobs, including mounted DMG contents/signature checks and Windows silent install, upgrade, GUI shutdown and uninstall. The Windows native/QEMU package, IPC, USB, reconnect, upgrade and uninstall procedure is documented in `docs/gui/windows-smoke.md`. Outstanding: publish through a new approved release, run that Windows smoke, add/run the corresponding native macOS smoke, then describe either host as supported only when its evidence passes
