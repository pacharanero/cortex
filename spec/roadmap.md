...
- [x] **GUI-004.2**: Defined the typed capability matrix in `gui/src-tauri/src/capability.rs`: `CapabilityStatus` (`confirmed-readable`, `confirmed-writable`, `inferred`, `unsupported`, `unverified`) defaults to `unverified`, and `CapabilityMatrix::status` returns that default for any operation not seeded - proven by a test keyed on an operation nobody has added yet, and by a second test proving an *empty* matrix confirms nothing. `default_matrix()` is seeded only from operations this file already records a hardware pass for: `switch_scene` and `recall_preset` (GUI-003.4 2026-08-16, MCP-003.1 2026-08-12, both `confirmed-writable`), and `block_parameters`/`set_parameter` (GUI-003.3 2026-08-17, `confirmed-readable`/`confirmed-writable`). A dedicated test proves `set_bypass`, `set_scene_label` and `set_scene_color` - implemented and offline-verified per GUI-003.3/GUI-003.4, exactly the trap this matrix exists to catch - stay `unverified`, because no hardware pass has confirmed them. **Outstanding:** the matrix is not yet consulted anywhere in the UI; that wiring waits for the hardware-faithful panel (GUI-002) and screen-reader surface (GUI-006) this labelling is meant to annotate
