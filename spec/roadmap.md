...
- [ ] **PROT-008.6**: `cortex connect` - a persistent, subscribed session. **In progress: the session holds and serves, the cache and health work do not exist yet.** Cortex Control is fast because it opens ONE session and keeps it; we pay a handshake per command. It is also the right shape for the MCP server, which must hold a single connection anyway, and for the GUI. Measured against a held session: `scene` 0.07 s, `grid` 0.14 s, `--status` 0.005 s, against a 3-5 s direct baseline.
  ...
  - [ ] **008.6.10**: **Handshake time varies hugely between identical runs.** Measured on a clean idle unit, no collisions, three consecutive start/stop cycles: **5.6 s, 22.9 s, 13.7 s**. A 4x spread, and not monotonic.
    - **This is almost certainly the same phenomenon as PROT-008.5**, which records a bare wait varying from 5 s to 49 s across identical runs because delivery is lazy. The observed handshake range (5.6-52.1 s) matches that range closely, and the handshake performs exactly those reads. Treat them as one problem with one fix - retry rather than wait - not two.
