...
- [x] **ENG-004.1**: Add `@see` traceability headers to all owned source files linking to zone specs. **Night: slice.** Cover one complete zone per PR and leave the parent planned until every owned source file is covered. `300-mcp` is fully covered (`crates/cortex-mcp/src/{main,server,transport}.rs`); `cortex-rs/src/` and `cortex-cli/src/` were already fully covered. `crates/cortex-host/src/` (owned by `200-cli`, including the `ipc/` adapter modules) was covered as of 2026-08-20. `gui/src-tauri/src/{main,lib,capability}.rs` (owned by `400-gui`) is now covered as of 2026-08-21, closing the parent: every non-test, non-build-script `.rs` file under a `src/` tree in the workspace carries a valid `@see`. **Scope note:** the governing acceptance criterion in `spec/001-overview/spec.md` (FR-1/NFR-1) is written against `.rs` files specifically, matching every zone covered so far; `gui/src/**/*.{ts,tsx}` frontend sources and `tests/`/`build.rs` files were not brought into scope by this item and would need their own convention decision before ENG-004.2's CI gate could cover them too
