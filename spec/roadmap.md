...
- [~] **PROT-004.4**: `view::Scene` exposes index, optional label and ARGB colour in every shared preset view; per-block scene bypass remains available through `view::Block::bypass`. A scene-centric aggregate over every block's bypass state remains planned only if a consumer needs it
...
- [x] **MCP-002.9**: Working-copy scene tools: `set_scene_label`, `unlabel_scene`, `set_scene_color`, `copy_scene`, `swap_scenes`. Hardware-verified through daemon protocol v5 on 2026-08-09; copy/swap perform a mandatory live-grid refresh because the device acknowledgement cannot distinguish them
