...
The typed read/edit, live-cache, health, reconnect, and shared prepared-save foundations are in place. The interactive Tauri first draft has explicit fixture and daemon-backed modes with generation-checked status, grid, scene, CPU and populated preset directory reads plus non-persistent working-state controls. Its production Rust boundary and physical unplug/reconnect behavior have passed against a real held session in the native Linux window; automated Tauri DOM/IPC checks and some rendered-control hardware smokes remain outstanding. Save controls remain disabled until the GUI implements exact-target preparation and confirmation UX, restoration semantics, typed failures and its own persistent-write hardware smoke. See [400-gui/spec.md](400-gui/spec.md).
...
- [ ] **ENG-004.1**: Add `@see` traceability headers to all owned source files linking to zone specs. **Night: slice.** Cover one complete zone per PR and leave the parent planned until every owned source file is covered. `300-mcp` is fully covered (`crates/cortex-mcp/src/{main,server,transport}.rs`); `cortex-rs/src/` and `cortex-cli/src/` were already fully covered. `crates/cortex-host/src/` (owned by `200-cli`, including the `ipc/` adapter modules) is now fully covered as of 2026-08-20. **Remaining:** `gui/`/`gui/src-tauri/src/` (owned by `400-gui`)
