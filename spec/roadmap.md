...
- [ ] **ENG-001.7**: Documentation quality checks for the docs site: a nav-orphan check, plus `s/linkcheck` and `s/spellcheck`. **Night: ready.** House style ([docs.md](/home/marcus/code/house-style/docs.md)) calls for these on a docs-heavy repo and is emphatic about the orphan check, because Zensical renders a page whether or not the nav references it - an orphaned page builds cleanly, reports no issues, and is reachable only by guessing its URL. Measured 2026-08-17: there are currently **no** orphans (13 pages, 13 nav entries) and **no** broken internal links across 50 files, so this is a regression guard rather than a bug fix, and it should go in while that is true. Wire into `s/lint` and CI as `s/markdownlint` is, keep the orphan check dependency-free (nav entries are the only `*.md` paths in `mkdocs.yml`), and record any deliberately unlisted page as a named exception carrying its reason, failing when an exception names a page that no longer exists
