...
- [ ] **PROT-008.6**: `cortex connect` - a persistent, subscribed session. **In progress: the session holds and serves, the cache and health work do not exist yet.** Cortex Control is fast because it opens ONE session and keeps it; we pay a handshake per command. It is also the right shape for the MCP server, which must hold a single connection anyway, and for the GUI. Measured against a held session: `scene` 0.07 s, `grid` 0.14 s, `--status` 0.005 s, against a 3-5 s direct baseline.
  ...
  - [ ] **008.6.10**: **Investigate handshake times escalating across sessions.** Measured 7.4 s, then 32.9 s, then 52.1 s on successive daemon starts, each separated by a collision. `pyquadcortex` previously ruled out plain reconnection as the cause (12 abandoned sessions, no degradation), which points at the collisions rather than the reconnects - but that is inference, not a controlled result. Needs a power-cycled baseline, then repeated clean start/stop cycles with no collision, to tell "collisions leave residue" from "our teardown does not fully release the device"
