...
- [x] **ENG-001.7**: Documentation quality checks for the docs site: a nav-orphan check, plus `s/linkcheck` and `s/spellcheck`. `s/check-docs-nav` and `s/linkcheck` are dependency-free bash, matching the same constraint the roadmap set for the orphan check: `s/check-docs-nav` extracts every `*.md` token from `mkdocs.yml`'s nav (nested sections included) and fails on a `docs/` page reachable by neither the nav nor a named `.nav-exceptions` entry (`<path><TAB><reason>`), and separately fails if a named exception no longer exists as a file. `s/linkcheck` resolves every `[text](target)` in `docs/` to a real file (skipping fenced code blocks and external `http(s)/mailto/tel` links) and, for a `#fragment`, checks it against the target heading's slug - either the Python-Markdown-style computed slug or an explicit `{#custom-id}` attr_list override, matching how Zensical itself resolves anchors. `s/spellcheck` runs pinned `codespell==2.4.1` against `docs/`, installing that version on demand the same way `s/markdownlint` pins its `npx` tool. Measured against the current tree: 13 docs pages, 0 orphans, 22 internal links, 0 spelling findings - all three pass clean today, confirming the roadmap's 2026-08-17 measurement rather than fixing a defect. All three are wired into `s/lint` (after `s/markdownlint`) and into dedicated `ci.yml` steps that also run their isolated-temp-repo fixture suites (`tests/check-docs-nav.sh`, `tests/linkcheck.sh`, `tests/spellcheck.sh`), following the `s/check-traceability` / `tests/check-traceability.sh` precedent. **Hardware follow-up: none** - offline docs tooling only. **Residual, not blocking:** this environment could not reach `/home/marcus/code/house-style/docs.md` to confirm the exact house-style tool choice/flags; `codespell` and the bash-only nav/link checkers were chosen to match this repo's existing conventions (pinned `npx`-style tools, dependency-free `check-*` scripts) rather than copied from that document - worth a quick diff against house style next time it's reachable
