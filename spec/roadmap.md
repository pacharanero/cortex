...
- [ ] **PROT-008.5**: Reduce the spread on a `File` READ listing, which varies from about 5 s to 50 s across identical runs because delivery is lazy.
  - **Retrying was tried and made it worse. Do not repeat it without a different mechanism.** Implemented as a re-fire of the READ every 3 s while a single waiter stayed registered (so no reply could be missed between attempts), then measured A/B over 5 runs each of `cortex presets`:
...
- [ ] **PROT-008.6**: `cortex connect` - a persistent, subscribed session. **In progress: the session holds and serves, the cache and health work do not exist yet.** Cortex Control is fast because it opens ONE session and keeps it; we pay a handshake per command. It is also the right shape for the MCP server, which must hold a single connection anyway, and for the GUI. Measured against a held session: `scene` 0.07 s, `grid` 0.14 s, `--status` 0.005 s, against a 3-5 s direct baseline.
  ...
  - [ ] **008.6.4**: **Health reporting.** Detect a dropped or unresponsive device and say so, rather than hanging. Reconnect with backoff, reporting each attempt.
    - **A fail-fast on silence was built and withdrawn. Do not rebuild it on the same premise.** The design was: while a wait is blocked, poll every 0.5 s and abandon the request once nothing at all has arrived for 5 s, on the reasoning that a `GlobalTempo` heartbeat every ~0.8 s makes silence conclusive. Purely observational, sending nothing. It refused a `version` request seconds after a clean handshake on a healthy unit.
    - Measuring instead of assuming: sampling time-since-last-message twice a second on an **idle** held session gave traffic for roughly 10 s after the handshake, then silence for 17 s in one run and 80+ s in another, reproduced on a fresh daemon with nothing else touching the device. Avoiding false positives would need a threshold over 90 s, which buys nothing over the existing 30 s request timeout.
    - This contradicts the earlier observation that `GlobalTempo` arrives every 0.8 s indefinitely - an observation solid enough at the time to have broken the adaptive settle. Both are real; what switches between them is unknown. See `spec/140-session/spec.md`. **Identify that first.** Any health signal built before then is guesswork.
  ...
  - [x] **008.6.9**: **Refuse to open the device while a daemon holds it.** Hardware-verified the hard way: a one-shot command that opened the device alongside the held session left every later read on that held session timing out. Nothing errored at the point of the collision - the damage only showed on the next request, which is what makes refusing loudly worth the code. `ensure_device_free()` is the CLI-side expression of the exclusive-access invariant. The socket is bound *before* the handshake, because the handshake window (33 s on an unsettled device) is when the daemon owns the interface but a late `bind()` would leave `is_running()` answering false.
    - The device also went quiet at that moment and this originally cited that as evidence. It no longer does: an idle session has since been measured silent for 80+ s while healthy (008.6.4), so silence distinguishes nothing. The read failures carry the finding on their own
  - [ ] **008.6.10**: **Handshake time varies hugely between identical runs.** Measured on a clean idle unit, no collisions, three consecutive start/stop cycles: **5.6 s, 22.9 s, 13.7 s**. A 4x spread, and not monotonic.
