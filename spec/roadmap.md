...
- [~] **NANO-001.5**: The GUI consumes the same `NanoCurrentState` through the daemon and renders the eight fixed roles as one responsive row (Gate, Pre FX 1-2, Capture, IR/Cab, Post FX 1-3) plus explicit Apply controls for the five raw amp values. It does not force Nano state into Quad rows/scenes/presets. The Tauri backend now reuses an existing compatible daemon or launches an auto-managed session itself; with both products connected the measured zero-setup path selected Quad, so an explicit in-GUI device selector remains outstanding. **Hardware-verified backend 2026-08-18:** the Tauri dashboard boundary read all eight roles through a real Nano daemon in 0.02 seconds from the paced held snapshot; a Tauri amp write changed Gain by one raw step, independently read it back, restored it and verified restoration in 12.15 seconds. Fictional Nano fixture mode at `?device=nano` passed responsive browser checks and an Apply interaction. MCP exposes `read_nano_state` and `set_nano_amp`; Quad tools invoked against a Nano daemon fail at the device boundary rather than pretending compatibility. **Outstanding:** add the dual-device selector, visually inspect the native Tauri Nano view and expose further writes only after each operation is safe on the held connection
