...
- [ ] **PROT-008.6**: `cortex connect` - a persistent, subscribed session. **In progress: the session holds and serves, the cache and health work do not exist yet.** Cortex Control is fast because it opens ONE session and keeps it; we pay a handshake per command. It is also the right shape for the MCP server, which must hold a single connection anyway, and for the GUI. Measured against a held session: `scene` 0.07 s, `grid` 0.14 s, `--status` 0.005 s, against a 3-5 s direct baseline.
  ...
  - [ ] **008.6.12**: **`CpuLoad` for a live DSP-load display.** Added to the subscribe set and exposed as `Session::cpu_load()` and `cortex cpu`, with a typed view carrying total load plus a per-column breakdown flagged by DSP core (`is_on_core2` - the QC splits the grid across two cores).
    - **The device does not push it to us, and we do not know why.** Verified on the wire: our `OUT CpuLoad` READ goes out during the subscribe burst, 2 bytes, and nothing comes back. Cortex Control's READ looks identical and it receives ~1 push per second (703 over its session, continuing through a 60 s idle). Raising the keepalive rate to match CC did not unlock it.
    - Candidates not yet tested: `CloudProduct`, which CC subscribes to and we do not; a difference in the READ's bytes rather than its shape; or a separate UI-state message that enables the feed - the schema has `Screenshot`, `ShowTuner` and `ShowGigView` adjacent to `CPULoad`, which suggests a family of display toggles. Compare the two READs byte for byte first, since that is cheapest and settles whether the request differs at all.
