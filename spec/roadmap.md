...
- [ ] **GUI-004.2**: Label hardware-verified vs provisional surfaces in the UI. Model this as a typed capability matrix (`confirmed-readable`, `confirmed-writable`, `inferred`, `unsupported`, `unverified`) whose default is tested to make no unsupported claims, rather than as ad-hoc copy ([prior-art.md](prior-art.md#the-idea-most-worth-stealing)). **Night: ready.** Define the matrix in Rust with `unverified` as the default, and a test proving an operation absent from the matrix is never rendered as confirmed - the default is the whole point, because an unlabelled surface is exactly the one likely to be wrong. Seed it from what this roadmap actually records as hardware-verified: preset recall, scene switching, and block parameter read/write are `confirmed-writable` where the entry says so; do not promote anything on the strength of it appearing to work
