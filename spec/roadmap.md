...
- [~] **PROT-006.11**: Capture and IR selection are HARDWARE-VERIFIED on CorOS 4.0.1. Capture selection preserved the exact reference and a post-selection parameter; user-IR selection preserved the exact key/name, and a timed on-unit inspection confirmed no warning. Both working-copy tests restored by recall. **Safety finding, 2026-08-12:** the on-unit action broadcast `try_to_show_dialog: true`; the exact false-only `NeuralCapture{UPDATE, show_dialog: false}` reply was sent; the unit froze, became silent on HID, and rebooted. Causation is not established, but the reply is unsafe, was removed from the public client and its smoke test, and must not be retried. Remaining silent suppresses the on-unit flow, so there is currently no safe host-connected unsupported response. Positive v1/v2 host capture remains PROT-006.17; payload transfer remains PROT-007
