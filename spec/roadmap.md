...
- [ ] **GUI-004.4**: `docs/gui/` explains how to use and run the GUI. **Night: ready.** Document what the GUI actually does today rather than what it is intended to become: fixture and daemon-backed modes and how to choose one, `s/gui-dev`, the preset directory and recall, the scene selector, and the parameter inspector. State plainly that recall replaces the working copy and that scene and parameter edits are non-persistent, since neither is obvious from the interface. Add the page to `mkdocs.yml` nav and build the Zensical site to prove it renders. Do not describe save controls, which do not exist
