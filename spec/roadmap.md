...
- [~] **CLI-004.14**: Native Tauri jobs build a Linux x86_64 `.deb`, Apple Silicon `.dmg` and Windows x86_64 NSIS `.exe`, each with its target-qualified `cortex` helper staged as a sibling binary. Windows uses current-user named-pipe ACLs and detached daemon startup; native host/CLI tests run before packaging. macOS is ad-hoc signed and Windows is unsigned, and both artifacts are labelled provisional. The Windows native/QEMU package, IPC, USB, reconnect, upgrade and uninstall procedure is documented in `docs/gui/windows-smoke.md`. Outstanding: pass the new remote matrix, publish through a new approved release, run that Windows smoke, add/run the corresponding native macOS smoke, then describe either host as supported only when its evidence passes
