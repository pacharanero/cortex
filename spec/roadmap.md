...
- [~] **NANO-001.2**: Offline implementation complete: framing and raw transport geometry are selected by `DeviceKind`; Nano uses hardware-verified PID `0x88E7` and 65-byte reports; Quad compatibility constants, 129-byte reports and benign write STALL remain intact; Nano write errors and all short writes propagate without retry; Quad `request` and `Session` reject Nano before USB I/O; one canonical byte-checked udev rule plus installer guidance covers both products; and focused no-HID tests now avoid workspace feature unification. Outstanding hardware smoke: open/read/write geometry on the reconnected Nano and rerun the Quad version/session smoke to prove no regression
