...
- [~] **NANO-001.6**: Nano working-state writes are being added one operation family at a time, with exact typed builders, hardware read-back through a separate state request, and unconditional restoration where a test changes a value. **Amp controls hardware-verified 2026-08-18:** Gain, Level, Bass, Mid and Treble each accepted a typed same-value write and independently read back exactly on one held connection in 30.28 seconds; a reversible Gain test also changed and restored the value. Protocol v15 exposes verified amp writes through daemon, CLI, MCP and GUI with no persistent save. **Bypass remains provisional:** Pre FX 1 changed and read back, but a second bypass write on the same HID handle was ignored despite pacing; reopening the handle allowed restoration. A response with footer `49 00 00 00` could also precede the requested state response, so the reader now skips unrelated command footers. Do not expose bypass until repeat writes are reliable, and do not expose save, preset switching, model replacement, Capture/IR selection or unverified parameter writes merely because prior art documents a frame
