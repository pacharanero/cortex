...
- [ ] **PROT-006.10**: File ops. **`save_current_preset` done** - captured from Cortex Control and implemented: `FileMessage{action: CREATE}` naming a destination slot in a `FolderInfo`/`ProductData`, with the name omitted to save in place and supplied to save-as or rename. It does NOT upload a preset; the unit commits the working grid. Guards refuse the factory library and a malformed slot before a session is opened. Outstanding: `delete_preset` (captured - `action: DELETE`, addressed by full path rather than slot index, see docs/protocol.md), `move_preset`, `copy_preset`, `create_setlist`, `delete_setlist`, `duplicate_setlist`
