...
- [ ] **ENG-004.1**: Add `@see` traceability headers to all owned source files linking to zone specs. **Night: slice.** `crates/cortex-rs/src/`, `crates/cortex-cli/src/`, `crates/cortex-host/src/`, `crates/cortex-mcp/src/`, and `gui/src-tauri/src/` now have top-level headers. **Remaining:** add governing-zone headers to the eight owned Rust build/test files reported by `rg --files-without-match '@see' --glob '*.rs'`; replace source links that point only to progress ledgers or moved roadmap IDs with stable `spec.md`/`design.md` targets; and explicitly settle whether FR-1's generic "source files" includes the owned TypeScript/frontend, scripts and configuration surfaces before declaring complete. The acceptance criterion currently requires every owned `.rs` file, including build scripts and tests; do not narrow that contract implicitly
- [ ] **ENG-004.2**: CI gate for `@see` link resolution. **Night: ready.** `deskop-nano-cortex/scripts/check-traceability.mjs` is a working reference: resolve document and node ID, fail broken links, warn missing back-references, and include an explicit config/workflow file list ([prior-art.md](prior-art.md#for-eng-004-and-eng-001)). Ship it as `s/check-traceability` wired into `s/lint` and CI, following the `s/markdownlint` shape. Gate only what exists: a file with no `@see` header is ENG-004.1's job, so the check must fail on a header that points at a missing document or node ID, not on a missing header, or it will fail the build for the zones still uncovered. Verify it by pointing a header at a deliberately bogus node ID and confirming a non-zero exit
