...
!!! warning "Residual verification for PROT-006.12"

    This item remains partial only for the exact visual checks named below. The completed PROT-006.7-.11 and PROT-006.13-.15 work retain per-operation evidence in [completed.md](completed.md) and [prior-art.md](prior-art.md#pyquadcortex---the-one-to-check-first); do not promote a residual from offline coverage alone.
...
- [~] **CLI-004.4**: Linux x86_64 cargo-dist release pipeline. The workflow validates its cargo-dist 0.32.0 plan, builds one combined archive containing both binaries, `SHA256SUMS`, licences/notices and `70-neural-dsp-cortex.rules`, and uploads it for pull-request/manual inspection. The auto-tag `workflow_call` path publishes that same checksum-verified archive and `SHA256SUMS` as a GitHub Release; the docs-root `install.sh` fetches only those release assets, verifies the archive entry, and installs both binaries. **CI and clean-environment evidence 2026-08-25:** PR #29's Ubuntu 22.04 preview job built the archive; its published SHA-256 entry verified after download; an isolated stock Ubuntu 22.04 container with the documented `xz-utils` prerequisite installed both binaries, ran `cortex 0.1.0`, generated the offline schema and confirmed that `cortex-mcp` emitted no non-protocol stdout on EOF. **Remaining:** exercise the auto-tag to GitHub Release publication path and run the public installer against those published assets. Add Linux aarch64 only after build and HID behaviour are verified; defer macOS/Windows until their local IPC, process lifecycle and hardware paths are implemented.
...
- [ ] **CLI-004.13**: Add changelog generation before the first public release. **Night: ready.** Resolve whether `git-cliff` or cargo-dist owns release notes, generate `CHANGELOG.md` in the version-bump flow, and make release automation consume that one source. House style names git-cliff ([commits.md](/home/marcus/code/house-style/commits.md)); confirm against the living example in `~/code/discourse/dsc` rather than reproducing a config from memory, and pin the tool version as `s/markdownlint` does. The repo already uses conventional commits, so verify the generated output against real history rather than assuming the default template suits it
...
- [~] **ENG-004.1**: Add `@see` traceability headers to all owned source files linking to zone specs. **Night: slice.** `crates/cortex-rs/src/`, `crates/cortex-cli/src/`, `crates/cortex-host/src/`, `crates/cortex-mcp/src/`, and `gui/src-tauri/src/` now have top-level headers. The stale-link sub-item is done: `s/check-traceability` (ENG-004.2) surfaced seven headers pointing at IDs that had moved or been split into subsection IDs during a design-doc restructure. `client.rs` and `session.rs` now use their closest living design anchors; the other five now point to their governing zone specs/designs rather than progress ledgers. Three pre-existing progress-only headers found during review also gained living targets. The eight owned Rust build/test files that `rg --files-without-match '@see' --glob '*.rs'` used to report - `crates/cortex-rs/{build.rs,tests/hardware-reads.rs}`, `crates/cortex-cli/tests/{lifecycle-hardware.rs,file-ops-process.rs}`, `crates/cortex-host/tests/{daemon-lifecycle-process.rs,daemon-protocol-skew-process.rs}`, `crates/cortex-mcp/tests/server-process.rs` and `gui/src-tauri/build.rs` - now name their governing living zones, with specific FR/DES IDs where a suitably narrow anchor exists, verified against `s/check-traceability`. `rg --files-without-match '@see' --glob '*.rs'` now reports nothing. **Remaining:** explicitly settle whether FR-1's generic "source files" includes the owned TypeScript/frontend, scripts and configuration surfaces before declaring complete. The acceptance criterion currently requires every owned `.rs` file, including build scripts and tests, which is now satisfied; do not narrow that contract implicitly when extending it to non-Rust surfaces
- [x] **ENG-004.2**: CI gate for `@see` link resolution. **Night: ready.** `s/check-traceability` parses every complete `@see <path> [ID...]` header in the tracked `.rs` tree: the target path must exist; every node ID must resolve literally as a structural bold roadmap/completed marker, bracketed design-doc heading, or requirements-table row; and every covered source file must name at least one living zone spec/design target. It gates only what exists - a missing header is ENG-004.1's job, not this check's - and is wired into `s/lint` and CI. `tests/check-traceability.sh` exercises all accepted forms plus missing paths/IDs, malformed syntax, trailing text, regex metacharacters, prose-only mentions, and progress-ledger-only links in isolated temporary Git repositories. Deliberately does not warn on a missing spec-to-source back-reference: only 8 of 13 zone `spec.md` files currently carry an "Owned source" line, so that check would flag zones that have not adopted the convention rather than a real defect - left as a residual if a future change wants it
...
**NANO-001.6 Gate-reduction update, 2026-08-22:** the exact 0-100% builder, daemon/CLI/MCP/Tauri routes, GUI Apply control, strict range rejection and fresh state confirmation are offline-tested. The connected unit omitted optional state field 53, which means unknown rather than 0%; the reversible smoke therefore refused before any write because it could not retain a trustworthy original value. This operation remains hardware-provisional until a readable original is changed, independently read back, restored and verified.
