...
- [~] **ENG-004.1**: Add `@see` traceability headers to all owned source files linking to zone specs. **Night: slice.** `crates/cortex-rs/src/`, `crates/cortex-cli/src/`, `crates/cortex-host/src/`, `crates/cortex-mcp/src/`, and `gui/src-tauri/src/` now have top-level headers. The stale-link sub-item is done: `s/check-traceability` (ENG-004.2) surfaced eight headers pointing at IDs that had moved to `spec/completed.md` or been split into subsection ids during a design-doc restructure (`client.rs`'s `[DES-CLIENT]` and `session.rs`'s `[DES-SESSION]` no longer exist as bare ids - only their `DES-CLIENT-*`/`DES-SES-*` children do), and all eight now point at a resolving target. **Remaining:** add governing-zone headers to the eight owned Rust build/test files reported by `rg --files-without-match '@see' --glob '*.rs'`; and explicitly settle whether FR-1's generic "source files" includes the owned TypeScript/frontend, scripts and configuration surfaces before declaring complete. The acceptance criterion currently requires every owned `.rs` file, including build scripts and tests; do not narrow that contract implicitly
- [x] **ENG-004.2**: CI gate for `@see` link resolution. **Night: ready.** `s/check-traceability` resolves every `@see <path> [ID...]` header in the tracked `.rs` tree: the target path must exist, and each node id must resolve in it as a bold roadmap/completed marker `**ID**`, a bracketed design-doc heading `## [ID] ...`, or an `| ID |` requirements-table row. It gates only what exists - a missing header is ENG-004.1's job, not this check's - and is wired into `s/lint` and CI (`ci.yml`, after the version-sync step) following the `s/markdownlint` shape. Verified by pointing a header at a deliberately bogus node id and confirming a non-zero exit, then reverting. Fixing the eight links it found live is filed under ENG-004.1 above rather than duplicated here. Deliberately does not warn on a missing spec-to-source back-reference: only 8 of 13 zone `spec.md` files currently carry an "Owned source" line, so that check would flag zones that have not adopted the convention rather than a real defect - left as a residual if a future change wants it
