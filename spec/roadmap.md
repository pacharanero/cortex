...
- [ ] **GUI-001.7**: Put an error boundary around the editing panels, so a frontend fault is visible instead of silent. **Night: ready.** **This has already bitten twice in one day:** a thrown React error unmounted `SceneSelector` while the daemon kept working, and it presented as "the keyboard stopped working" - writes were still reaching the device, so nothing looked broken except that the controls had gone. A Rules-of-Hooks violation in the parameter editor would have done the same. Wrap the grid, scene selector and inspector so a fault renders a visible failure naming the component and offering a reload, rather than a blank space. Prove it with a test component that throws, and check the boundary does not swallow the error from the console - a silent boundary trades one invisible failure for another
