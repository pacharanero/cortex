...
- [ ] **ENG-005.3**: Decode the protobuf body per message type, not just the type tag and size. Optionally as a Wireshark Lua dissector, since Wireshark's built-in protobuf support can be pointed at our vendored `.proto` files - worth it only if the GUI earns its place alongside `--live`
