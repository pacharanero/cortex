...
- **Release pipeline is incomplete.** Post-gate auto-tagging, the Linux preview, protected GitHub Release hosting and retry repair exist. The release workflow accepts an existing tag on explicit dispatch so a workflow defect can be fixed before rebuilding and repairing that release through the same protected hosting job. Crates.io publishing, GUI bundles, and a live auto-tag-to-release smoke remain.
