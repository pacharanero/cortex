...
- **Nano Cortex application coverage remains partial.** Geometry and raw framing are shared, while the implemented Nano codec parses its four-byte footer separately from `Message::parse`'s Quad-specific eight-byte trailer. Typed state plus amp, bypass and raw FX parameter operations are implemented; wider application operations remain outstanding. Quad request/session entry points reject Nano before USB I/O so the envelopes cannot be conflated.
